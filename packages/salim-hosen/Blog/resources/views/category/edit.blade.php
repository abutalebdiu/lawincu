...
@section("meta_tags")
    ...
    <meta name="description" content="Edit Category and Manage Category Details">
    <meta name="keywords" content="category,category_edit">
@endsection
    ...
    <div class="card-body">
        <form action="{{ route('blog-categories.update', $blogCategory->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $blogCategory->name) }}">
                @error('name')
            ...
            <div class="mb-3">
                ...
                <input class="form-control {{ $errors->has('arabic_name') ? 'is-invalid' : '' }}" type="text" name="arabic_name" id="arabic_name" value="{{ old('arabic_name', $blogCategory->arabic_name) }}">
                @error('arabic_name')
                    <div class="invalid-feedback">
                        {{$message}}
                    </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category" id="category">
                    ...
                    @foreach ($categories as $cat)
                        @if ($blogCategory['id'] != $cat['id'])
                        <option @if(isset($parent_category['id']) && $parent_category['id'] == $cat['id']) selected @endif value="{{ $cat['id'] }}">{{ $cat['name'] }}</option>
                        @endif
                    @endforeach
                </select>
                @error('category')
